...
```
  ...
  Android has specific build requirements, which are spelled out in
  "Establishing a Build Environment":
      https://source.android.com/source/initializing
   When Building on an Ubuntu 17.04 build system, the following
   other packages may be needed:
```shell
      sudo apt-get install openjdk-8-jdk flex bison curl ccache git-core gnupg
      sudo apt-get install gperf build-essential libc6-dev-i386 libxml2-utils
```
...
```
  ...
  We use a different version of Linux to support Android, derived
  from the HiSilicon SDK.  We just use the master branch from this
  tree.
...
### Step 4: Get the source code for Android with Poplar support.
  We will use Android AOSP version 7.1.1_r3 as a base.  (Downloading
  this will take at least an hour, probably more.)
...
```shell
    # This produces two output files, which are used when building
    # the USB flash drive image.  The DTB file will be renamed.
    #       arch/arm64/boot/Image
    #       arch/arm64/boot/dts/hisilicon/hi3798cv200.dtb
    cd ${TOP}/linux
...
### Step 5: Build Android, using the Linux we just built
  First we need to copy the prebuilt kernel image, the device tree
  file, and all kernel modules (only one at the moment) into place
  in the Android tree.  (Note the DTB is renamed.)

```shell
    cd ${TOP}/android
    mkdir -p device/hisilicon/poplar-kernel
    cp ${TOP}/linux/arch/arm64/boot/Image \
		device/hisilicon/poplar-kernel/Image
    cp ${TOP}/linux/arch/arm64/boot/dts/hisilicon/hi3798cv200.dtb \
		device/hisilicon/poplar-kernel/hi3798cv200-poplar.dtb
    cp ${TOP}/linux/drivers/bluetooth/mt76x2u/btmtk_usb.ko \
		device/hisilicon/poplar-kernel/modules/btmtk_usb.ko

```
  Finally we need to build Android.  This takes a very long time,
...
### Step 6: Gather the required components you built above
  Gather the files that will be required to flash a Poplar board
  with the images that support Android.

```shell
    mkdir -p ${TOP}/recovery
    cd ${TOP}/recovery
...
### Step 7: Build image files used for recovery and installation
  ...
  Running "poplar_recovery_builder.sh" will create a directory
  containing files used to recover a Poplar board.  Included is
  "fastboot.bin", which can be put on a USB flash drive to boot
  a "bricked" Poplar board.  In addition there are U-Boot scripts
  named "install*" which install the (partial) partition images 
  onto Poplar eMMC.  Supplying "android" as the only argument
  to "poplar_recovery_builder.sh" produces files used to build
  an Android system on a Poplar board.
```shell
    # This produces a directory "recovery_files", containing
    # "fastboot.bin" as well as other files used to populat the
    # eMMC media with a bootable Android system.
...
- Inform U-Boot about the network parameters to use.  Use values for
  ...
  configure the Ethernet interface.  Again:  YOU WILL NEED TO
  SUPPLY VALUES HERE APPROPRIATE FOR YOUR NETWORK.
```shell
...
```
  ...
  Currently, Android does not automatically boot following a reset.
  Interrupt the automatic boot (with ^C if necessary).  Once at the
  "poplar# " prompt, enter this command to boot Android from the
  eMMC:
```shell
    run bootai
```
